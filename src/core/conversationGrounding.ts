...
export type ConversationalIntent = 'scene_overview' | 'situation_recap';
...
export function renderIdentityArrival(input: string, role: PlayerClassId): string {
  ...
  if (/دوست|رفیق|خاطره|قدیمی/.test(norm)) {
    ...
    grounding = `سالار دلیل خودش را برای اعتماد به تو دارد: «${cleanIdentityForNarrative(input)}». شاید تخصص کلاسیکی نداشته باشی، اما بدون پیش‌فرض وارد ماجرا می‌شوی و مسیرت را با انتخاب‌هایت می‌سازی.`;
  }
...
export function renderSceneOverview(state: RunState, intent: ConversationalIntent): string {
  ...
  switch (sceneId) {
    case 'scene_intro':
      return isRecap
        ? 'سالار توضیح کامل نداده؛ فقط گفته پای گذشته و یک تابلو در میان است و باید همین حالا به پلاک ۵۵ برسی. هنوز می‌توانی هر وقت خواستی بگویی چرا از میان همه سراغ تو آمده—اما جهان منتظر این جواب نمی‌ماند.'
        : 'بیرون کافه‌ای. از شکاف در صدای کوتاه بخار و برخورد فنجان می‌آید و چند سایه در سالن حرکت می‌کنند؛ کنار آستانه رسیدی نم‌کشیده افتاده و مرد پالتوپوشی با دستکش قرمز در حال دور شدن است. می‌توانی پیشینه‌ات را بگویی یا مستقیم با هرکدام درگیر شوی.';
    case 'scene_entrance':
      return isRecap
        ? 'سالار پشت تلفن همه‌چیز را نگفت. فعلاً فقط این‌ها را می‌دانی: نیمه‌شب تو را به پلاک ۵۵ کشانده، گفته پای گذشته و تابلو در میان است، و درست حالا مردی با پالتوی تیره از کافه دور می‌شود. کنار در، رسیدی نم‌کشیده روی سنگ‌فرش افتاده و داخل کافه از پشت شیشه روشن است.'
        : 'زیر نور زرد سردر، سه چیز در دید توست: درِ شیشه‌ای کافه که هنوز تکان می‌خورد، رسید نم‌کشیده‌ای کنار آب باران، و مرد پالتوپوشی که با دست راست پوشیده در حال دور شدن است. از شکاف در صدای کوتاه بخار و برخورد یک فنجان می‌آید و چند سایه در سالن حرکت می‌کنند؛ چهره‌ها و جزئیات میزها از آستانه روشن نیست.';
    case 'scene_table5':
      return isRecap
    ...
    case 'scene_collector_meeting':
      return 'خط امن نمایندهٔ خریدار باز است و سالار کنار میز ملاقات نشسته. اینجا دیگر مسئله فقط کشف سرنخ نیست؛ هر ادعا، پیشنهاد یا تهدید می‌تواند اختیار تابلو و آدم‌های کافه را تغییر دهد.';
    case 'scene_archive':
      return 'مدارکی که واقعاً پیدا کرده‌ای روی میز آرشیو قرار دارند و سالار روبه‌رویت منتظر است. جاهای خالی خط زمانی هنوز خالی‌اند؛ هیچ برگه‌ای صرفاً به‌خاطر رسیدن به این اتاق کامل نشده است.';
